...
In [a previous article][TSB001], I introduced a new measure of disorder that I called $$\mathit{Amp}$$, and attempted to determine whether it was a measure of presortedness.
...
A counterexample can be constructed as follows: $$\mathit{Amp}$$ considers a single descending run to be sorted, but two descending runs, the second greater than the first, not sorted:
* ‎$$\mathit{Amp}(\langle 4, 3, 2, 1, 0 \rangle) = 0$$
* ‎$$\mathit{Amp}(\langle 9, 8, 7, 6, 5 \rangle) = 0$$
* ‎$$\mathit{Amp}(\langle 4, 3, 2, 1, 0, 9, 8, 7, 6, 5 \rangle) = 2$$
...
$$X$$ has high disorder:
* $$\mathit{Block}(X) = 10 \: (10)$$, the number of elements in $$X$$ that are not followed by the same element in the sorted permutation.
* $$\mathit{Dis}(X) = 9 \: (9)$$, the maximum distance determined by an inversion.
* $$\mathit{Exc}(X) = 5 \: (10)$$, the minimum number of exchanges required to sort $$X$$.
* $$\mathit{Ham}(X) = 10 \: (10)$$, the number of elements in $$X$$ that are not in their sorted position.
* $$\mathit{Inv}(X) = 45 \: (45)$$, the number of inversions in $$X$$.
* $$\mathit{Max}(X) = 9 \: (9)$$, the maximum distance an element in $$X$$ must travel to find its sorted position.
* $$\mathit{Rem}(X) = 9 \: (9)$$, the minimum number of elements that must be removed from $$X$$ to obtain a sorted subsequence.
* $$\mathit{Runs}(X) = 10 \: (10)$$, the number of increasing runs in $$X$$.
* $$\mathit{Spear}(X) = 50 \: (50)$$, the sum of distances between the position of individual elements in $$X$$ and their position in its sorted permutation.
* $$\mathit{SUS}(X) = 10 \: (10)$$, the minimum number of increasing subsequences (of possibly non-adjacent elements) into which $$X$$ can be decomposed.

A first observation here is that we find all measures of disorder based on the sorted position of the elements ($$\mathit{Block}$$, $$\mathit{Dis}$$?, $$\mathit{Exc}$$, $$\mathit{Ham}$$, $$\mathit{Max}$$ and $$\mathit{Spear}$$),
as well as those that look for exclusively ascending sequences of some sort ($$\mathit{Rem}$$, $$\mathit{Runs}$$ and $$\mathit{SUS}$$).

$$X$$ has low disorder:
* $$\mathit{Amp}(X) = 0 \: (8)$$, the amplitude of the prefix sum of pairwise difference (see [here][TSB001]).
* $$\mathit{Enc}(X) = 1 \: (5)$$, the number of encroaching lists that can be extracted from $$X$$.
* $$\mathit{Mono}(X) = 1 \: (5)$$, the number of increasing and decreasing runs in $$X$$.
* $$\mathit{Osc}(X) = 0 \: (40)$$, the oscillation of $$X$$[^1].
* $$\mathit{SMS}(X) = 1 \: (4)$$, the minimum number of increasing or decreasing subsequences (of possibly non-adjacent elements) into which $$X$$ can be decomposed.

Here we have two main subcategories of measures: those that explicitly look for ascending and descending sequences ($$\mathit{Enc}$$, $$\mathit{Mono}$$ and $$\mathit{SMS}$$),
and those where the result naturally arises from a symmetry between a sequence, and a similar sequence in reversed order ($$\mathit{Amp}$$ and $$\mathit{Osc}$$).
...
and all but $$\mathit{Enc}$$ are actually perfectly symmetric over the input sequence[^2][^3]: accepting any sequence forward or backward returns the same result.
...
the same way $$\mathit{Runs}$$ is fixed by returning the number of runs minus $$1$$ (or is redefined as returning the number of "steps-down" in the sequence).
...
In *Sorting and Measures of Disorder*, Vladimir Estivill-Castro recognizes the issue of $$\mathit{Enc}$$ returning $$1$$ for an ascending run,
as well as the shortcomings of trying to use $$\mathit{Enc}(X) - 1$$ to circumvent the issue.
Instead he proposes a new equivalent measure based on $$\mathit{Enc}$$, dubbed $$M_{\mathit{Enc}}$$, which satisfies all five of Mannila's axioms and is thus a proper measure of presortedness:
...
* $$M_{\mathit{Amp}}$$ does not satisfy axiom 4: $$M_{\mathit{Amp}}(\langle 2, 1, 0 \rangle) = 1$$, $$M_{\mathit{Amp}}(\langle 4, 3 \rangle) = 1$$, but $$M_{\mathit{Amp}}(\langle 2, 1, 0, 4, 3 \rangle) = 3$$.
* $$M_{\mathit{Mono}}$$ does not satisfy axiom 4: $$M_{\mathit{Mono}}(\langle 1, 0 \rangle) = 1$$, $$M_{\mathit{Mono}}(\langle 2, 3 \rangle) = 0$$, but $$M_{\mathit{Mono}}(\langle 1, 0, 2, 3 \rangle) = 2$$.
* $$M_{\mathit{Osc}}$$ does not satisfy axiom 4: $$M_{\mathit{Osc}}(\langle 1, 0 \rangle) = 0$$, $$M_{\mathit{Osc}}(\langle 3, 2 \rangle) = 0$$, but $$M_{\mathit{Osc}}(\langle 1, 0, 3, 2 \rangle) = 1$$.
* Computing $$\mathit{SMS}$$ is an NP-complete problem[^5], so I can't reasonably experiment with $$M_{SMS}$$.
...
## Notes
  ...
  [^1]: $$\mathit{Osc}(X)$$ is formally defined as $$\sum_{i=0}^{\lvert X \rvert - 1} \vert \vert \mathit{Cross}(x_i) \vert \vert$$
        ...
        In the following image, $$\mathit{Cross}(x_5) = \{ 1, 3, 6 \}$$, and $$\mathit{Osc}(X) = 17$$:
        ![Plot of Cross(x5) over the sequence 6, 3, 9, 8, 4, 7, 1, 10]({{ site.baseurl }}/assets/images/TSB006/osc-example.png){:.centered}

  [^2]: $$\mathit{Enc}(\langle 4, 5, 3, 6, 2, 7, 1, 8, 0, 9 \rangle) = 1$$, but $$\mathit{Enc}$$ returns $$5$$ (its maximum) over the same sequence reversed.

  [^3]: The symmetry of $$\mathit{Osc}$$ is mentioned in *Adaptive Heapsort* by C. Levcopoulos and O. Petersson,
        the symmtry of $$\mathit{Amp}$$ is proven in my previous article [*Symmetry of Amp*][TSB005],
        the symmetry of $$\mathit{Mono}$$ and $$\mathit{SMS}$$ are only conjectured as far as I know but arise intuitively from their definition.
